...
$exam_stmt = $conn->prepare("SELECT * FROM exam_routine WHERE class = ? AND section = ? ORDER BY exam_date, start_time");
$exam_stmt->bind_param("ss", $class, $section);
$exam_stmt->execute();
...
<body>
    ...
    <div class="container" id="exam-section">
        <div class="header-print">
            <strong>IUBAT – International University of Business Agriculture and Technology</strong><br>
            <small>Exam Routine • Generated on <?php echo date("F j, Y"); ?></small>
        </div>

        <h2>Exam Routine for Class <?php echo htmlspecialchars($class); ?> (Section <?php echo htmlspecialchars($section); ?>)</h2>
        ...
        <?php else: ?>
            <div class="no-data">No exam routine found for your class and section.</div>
        <?php endif; ?>
    ...
    <script>
        function downloadPDF() {
            const element = document.getElementById('exam-section');
            ...
            const options = {
                margin: 0.5,
                filename: 'IUBAT_exam_routine.pdf',
